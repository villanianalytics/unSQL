...
public class DifferentTest {
	...
	@Before
	public void init() {
		different = new Different(TestUtils.createElementsExtract());
	}

	@Test
	public void testInvalidWhere() {
		boolean valid = different.isValid("");
		assertFalse(valid);
	}

	@Test
	public void testValidWhere() {
		boolean valid = different.isValid("test.test<>1");
		assertTrue(valid);
	}

	@Test
	public void testValidPredicateNumeric() {
		Predicate<String> predicate = different.generate("test.test<>1");
		assertNotNull(predicate);

		List<String> objects = Arrays
				.asList(new String[] { "test.test=1", "testsss.test=1", "testa=1", "test[1].test=1" });
	...
	public void testValidPredicateString() {
		Predicate<String> predicate = different.generate("test.test<>'2'");
		assertNotNull(predicate);
	...
	public void testValidPredicateField() {
		Predicate<String> predicate = different.generate("test.test<>test.test");
		assertNotNull(predicate);
