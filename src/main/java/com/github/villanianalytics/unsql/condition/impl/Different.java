...
public class Different implements Condition {
	...
	/** The Constant DIFFERENT_PATTERN. */
	private static final Pattern DIFFERENT_PATTERN = Pattern.compile(".*<>.*");

	/** The Constant DIFFERENT_DELIMITER. */
	private static final String DIFFERENT_DELIMITER = "<>";

	/** The elements extract. */
	private ElementsExtract elementsExtract;
	
	/**
	 * Instantiates a new different.
	 *
	 * @param elementsExtract the elements extract
	 */
	public Different(ElementsExtract elementsExtract) {
	...
	public boolean isValid(String whereClause) {
		if (whereClause == null)
			return false;
		Matcher m = DIFFERENT_PATTERN.matcher(whereClause);
		return m.matches();
	}
	...
	public Predicate<String> generate(String whereClause) {
		return input -> {
			List<String> valOnes = elementsExtract.getElementValue(input, whereClause, DIFFERENT_DELIMITER, 0);
			List<String> valTwos = elementsExtract.getElementValue(input, whereClause, DIFFERENT_DELIMITER, 1);
